...
from engine_for_vqvae import evaluate, train_one_epoch
...
def main(args, ds_init):
    ...
    total_batch_size = (
        args.batch_size * args.update_freq * utils.get_world_size()
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None and (
            epoch % 10 == 0 or epoch + 1 == args.epochs
        ):
            test_stats = evaluate(data_loader_val, model, device)
            print(
                f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['iou']:.1f}%"
            )
            ...
            if log_writer is not None:
                log_writer.update(
                    test_iou=test_stats["iou"], head="perf", step=epoch
                )
                log_writer.update(
